    ...
    12.times do
      with_fx :gverb, room: 60, release: 1, mix: 0.2 do
        with_fx :slicer, phase: 0.25*3, wave: 0, pulse_width: 0.666, mix: 1 do
          ...
          synth :tb303, note: ring(56,54,54,52,52,54,54,52,54,52,49,49).look(:ii)-12, amp: 2, detune: 16, sustain: 4, cutoff: 85
        end
      end
...
in_thread do
  live_loop :tickas do
    with_fx :reverb, room: 0.3, mix: 0.4, amp: 0.3 do
      8.times do
        synth :dsaw, release: rrand(0.15,0.25), amp: 3, env_curve: 3, res: 0.33, cutoff: rrand(105,115)
        synth :dsaw, release: rrand(0.15,0.25), amp: 3, env_curve: 2, res: 0.43, cutoff: rrand(105,115)
